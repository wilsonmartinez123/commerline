...
if (is_array($array_data)) {
    ...
    foreach ($array_data as $value) {
        ...
        $sqlte = "SELECT * FROM categorias WHERE nombre_categoria = '$categoria'";
        $resultado = mysqli_query($con, $sqlte);
        $fila = mysqli_fetch_array($resultado);

        $IdCategoria = $fila['id_categoria'];

        //seleccionar el ID de la subcategoria escogida
        $subcategoria = utf8_decode($value["subcategoria"]);
        $sqlte = "SELECT * FROM subcategorias WHERE nombre_subcategoria = '$subcategoria' ";
        $resultado = mysqli_query($con, $sqlte);
        $fila = mysqli_fetch_array($resultado);

        $IdSubcategoria = $fila['id_subcategoria'];
        ...
        if (!empty($data["diasOferta"])) {
            ...
            if (!file_exists($filename)) {
                ...
                $sql = "INSERT INTO productos (id_empresa, nombre_pro, imagen_pro, precioNuevo_pro, descripcion_pro, fecha_registro_pro, id_categoria, id_subcategoria, precioAnterior_pro, diasOferta_pro, inicioOferta_pro, finOferta_pro ) VALUES ('$IDempresa','$name', '$images'    ,  '" . $value["price"] . "',  '$description', '$created',  '$IdCategoria' ,  '$IdSubcategoria' ,  '" . $value["precioOferta"] . "',  '$diasOferta', '$inicioOferta', '$finOferta')";
                $query = $con->query($sql);
            ...
            if (!file_exists($filename)) {
                ...
                $sql = "INSERT INTO productos (id_empresa, nombre_pro, imagen_pro, precioNuevo_pro, descripcion_pro, fecha_registro_pro, id_categoria, id_subcategoria) VALUES ('$IDempresa','$name', '$images'    ,  '" . $value["price"] . "',  '$description', '$created',  '$IdCategoria', '$IdSubcategoria')";
                $query = $con->query($sql);
