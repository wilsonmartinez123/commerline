...
/********************************************************************************************** */
       ...
       // file_put_contents('uploads/img_'.date('Y-m-d-H-s').'.jpg', base64_decode(str_replace(' ','+',$image)));
       ...
       $images = ('uploads/img_'.date('Y-m-d-H-s').'.jpg');
          ...
          try {
            $sql = "INSERT INTO productos(name, image_name, price, description) VALUES(:name, :image, :price, :description)";
             ...
             $stmt->bindParam(':image', $images, PDO::PARAM_STR);
      ...
      case "update":
        ...
        $price 		     = filter_var($obj->price, FILTER_SANITIZE_STRING, FILTER_FLAG_ENCODE_LOW);
         ...
         try {
            $sql 	= "UPDATE productos SET name = :name, image_name = :image, price = :price, description = :description WHERE id = :recordID";
            $stmt 	=	$pdo->prepare($sql);
            $stmt->bindParam(':name', $name, PDO::PARAM_STR);
            $stmt->bindParam(':image', $images, PDO::PARAM_STR);
            $stmt->bindParam(':price', $price, PDO::PARAM_STR);
